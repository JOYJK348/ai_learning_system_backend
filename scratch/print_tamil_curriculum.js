...
async function main() {
  ...
  console.log('Tamil Subjects found:');
  for (const sub of subjects || []) {
    console.log(`Subject ID: ${sub.id}, Name: ${sub.name}, Grade: ${sub.grades?.name}`);
  }

  const tamilGrade1 = subjects.find(s => s.grades?.name === 'Grade 1');
  if (tamilGrade1) {
    console.log(`\nFetching chapters and lessons for Grade 1 Tamil (${tamilGrade1.id})...`);
    
    const { data: chapters } = await supabase
      .from('chapters')
      .select('id, name, sort_order')
      .eq('subject_id', tamilGrade1.id)
      .is('deleted_at', null)
