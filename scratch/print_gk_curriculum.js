...
async function main() {
  const { data: subjects } = await supabase
    ...
    .ilike('name', '%knowledge%')
    .is('deleted_at', null);

  console.log('GK Subjects found:');
  for (const sub of subjects || []) {
    console.log(`Subject ID: ${sub.id}, Name: ${sub.name}, Grade: ${sub.grades?.name}`);
  }

  const gkGrade1 = subjects.find(s => s.grades?.name === 'Grade 1');
  if (gkGrade1) {
    console.log(`\nFetching chapters and lessons for Grade 1 GK (${gkGrade1.id})...`);
    
    const { data: chapters } = await supabase
      .from('chapters')
      .select('id, name, sort_order')
      .eq('subject_id', gkGrade1.id)
      .is('deleted_at', null)
